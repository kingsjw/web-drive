<?php
  ...
  $rs = $db->query($sql);
  $sql = "SELECT * FROM outside WHERE u_idx='{$_SESSION['user']['idx']}'";
  $row = $db->query($sql);
?>
<form class="container pt-5" action="/control/user/outside_remove" method="post">
    <h2>외부공유 목록</h2>
    <hr>
    <h5 class="mt-5">
    ...
    <table class="table table-hover table-bordered mt-3">
        ...
        ?>
          <tr>
              ...
              <td><?=$data['wdate']?></td>
              <td><?=number_format($data['down_cnt'])?></td>
              <td><a href="/?q=<?=$data['code']?>">http://localhost?q=<?=$data['code']?></a></td>
          </tr>
        <?php }?>
        </tbody>
    </table>
